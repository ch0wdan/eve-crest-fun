...
module.exports = function (models, baseClass) {
  ...
  MarketType.clean = function (row) {
    console.log("ORIG", row);
    row.bid = (row.bid === 'True') ? 1 : 0;
    ['typeID', 'orderID', 'stationID', 'regionID',
      'solarSystemID'].forEach(function (name) {
      row[name] = '' + row[name];
    });
    ['price', 'volRemaining', 'volEntered', 'range',
      'minVolume', 'duration'].forEach(function (name) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      return Promise.all(_.map(ordersByRegionAndType, function (orders, key) {
        ...
        return models.MarketType.get({
          ...
          obj.sellOrders = ordersByBid['0'] || [];
          obj.buyOrders = ordersByBid['1'] || [];
          return obj;
        }).then(function (obj) {
